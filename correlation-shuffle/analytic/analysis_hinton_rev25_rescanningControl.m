...
within_all = zeros(3,3);
between_all = zeros(3,3);
...
BAS_FLAG = 1;
nrep=1; % repetitions of splitting i.e. bootstrap samples
...
for j=1:3  %patchy  stripy-h stripy-v
    fprintf('pattern%d in process.\n', j);
    if BAS_FLAG
        data1=REV(j,subj_nr,1);  %pull out data  subjects
        data2=REV(j,subj_r,2);  %pull out data   BAS
...
fprintf('parti_nr%d, parti_r%d: within corr=%f;\n', subj_nr, subj_r, within);
fprintf('parti_nr%d, parti_r%d: betwee corr=%f;\n', subj_nr, subj_r, between);

within_all(subj_nr,subj_r) = within;
between_all(subj_nr,subj_r) = between;
