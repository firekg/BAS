...
for subj=1:4
    ...
    for j=1:3  %patchy  stripy-h stripy-v
        ...
        for k=1:data.Trials %trials
            ...
            trial.rev(tcount)=data.MaxRevealingTrial(k);
            ...
            for rev=1:data.MaxRevealingTrial(k) % loop over revealings for the trial
                ...
                fix.im_type(count)=j;
                fix.rev_num(count)=rev; % revealing number
                fix.trial(count)=count; % trial number
    ...
    %%
    S=bindata([trial.image ;trial.rev]',ones(cols(trial.image),1));
    ...
    for i=1:nimage
        mask((i-1)*nmin+1:i*nmin,i)=((nimage-1)/ntotal);
    end
    ...
    for reshuffle=0
            ...
            for j=1:3
                ...
                if BAS_FLAG
                    ...
                    for k=[5:5:25]  %make this even for each
                        s=find(trial.image==j & trial.rev==k);
                ...
                for k=1:2
                    ...
                    for rev=minrev:nrev
                        if reshuffle  %this is for reshuffling
                            ...
                            e=[];
                            for i=1:rev
                                ps=p.* (fix.rev_num==i); %choose all the fixations from these trials with weights
                                e=[e datasample(1:length(p),floor(nmin/rev),'Replace',false,'Weights',ps+eps)];
                            end
                            miss=nmin-length(e);
                            e=[e datasample(1:length(p),miss,'Replace',false,'Weights',p+eps)];
                        end
        ...
        hold on
        errorbar(minrev:25,mean(within(minrev:end,:)'),std(within(minrev:end,:)'),'g');
        errorbar(minrev:25,mean(between(minrev:end,:)'),std(between(minrev:end,:)'),'r');
